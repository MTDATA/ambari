...
public class MapReduceJobHistoryUpdater implements LogStoreUpdateProvider {
  ...
  private PreparedStatement etlWorkflowPS = null;
  ...
  private void updateJobStatsAtFinish(String jobId) {
    try {
      ...
      LOG.error("Failed to update mapsRuntime/reducesRuntime for " + jobId, sqle);
    }
  }
  ...
  private void processJobSubmittedEvent(
    ...
    try {
      ...
      try {
        ...
        LOG.error("workflow select failed with: ", sqle);
      ...
      if (insertWorkflow) {
        ...
        try {
          ...
          LOG.error("Couldn't read existing workflow context for " + workflowContext.getWorkflowId(), e);
        }
      ...
      if (historyEvent.getWorkflowTags() != null && !historyEvent.getWorkflowTags().trim().isEmpty()) {
        Map<String, String> workflowTags = getWorkflowTags(historyEvent);
        etlWorkflowPS.setString(1, workflowContext.getWorkflowId());
        etlWorkflowPS.setBytes(2, longToByteArray(new BigInteger(workflowTags.get(FLOW_ID), 16).longValue()));
        try {
      ...
      LOG.error("Failed to store " + historyEvent.getEventType() + " for job " +
  ...
  private void processTaskStartedEvent(PreparedStatement entityPS,
    ...
    try {
      ...
      LOG.error("Failed to store " + historyEvent.getEventType() + " for task " +
  ...
  private void processTaskAttemptStartedEvent(
    ...
    try {
      ...
      LOG.error("Failed to store " + historyEvent.getEventType() +
...
//        historyEvent.getTaskType());
  ...
  public static byte[] longToByteArray(Long l) {
    ...
    for(int i = 0; i < 8; i++) output[i] = (byte)((l >> ((7 - i) * 8)) & 0xff);
    return output;
  }
